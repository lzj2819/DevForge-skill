...
See `skill/tools/precondition-checker.md`. Acceptable phases: `scaffolding_completed`, `module_design_completed`.
...
1. **Load parent context**
   - Read `PROJECT_SCAFFOLD/docs/architecture/system/PRD.md` (full system requirements)
   - Read `PROJECT_SCAFFOLD/docs/architecture/system/architecture.xml` (system-level architecture)
   - Read `PROJECT_SCAFFOLD/docs/architecture/system/INTERFACE_CONTRACT.md` (system-level I/O boundaries)
   - Read `PROJECT_SCAFFOLD/docs/architecture/system/DECISION_LOG.md` (reasoning chain)
   - Read `PROJECT_SCAFFOLD/docs/architecture/system/STATE.md` (module registry, if any modules already designed)
...
6. **Module-level XML modeling**
   ...
   - For each component, generate a `component-spec.xml` template at `PROJECT_SCAFFOLD/docs/architecture/modules/{module_id}/components/{component_id}/component-spec.xml`
...
8. **Component code skeleton generation**
   - **Override strategy**: Scaffolding generates `__init__.py` with only module header comments (no business logic). Module-design overwrites these safely. If a manual modification marker (`<!-- MANUAL -->` or `# MANUAL:`) is detected, preserve user changes and append new content.
...
11. **State Update**
    ...
    See `skill/tools/state-updater.md`. This skill transitions phase to `module_design_completed`.
    ...
    - Update `PROJECT_SCAFFOLD/docs/architecture/system/STATE.md`:
      ...
      - Update **Module Registry**: append `{id: module_id, status: design_completed, path: PROJECT_SCAFFOLD/docs/architecture/modules/{module_id}/}`
      - Append any module-specific risks to **Known Pitfalls**
    - Update `PROJECT_SCAFFOLD/docs/architecture/INDEX.md`: append module row with links to generated artifacts

<HARD-GATE>
Do NOT mark the module design as complete or allow transition to test-execution until the user replies [APPROVE] or explicitly asks to continue. Module design must be approved before testing begins.
</HARD-GATE>
...
2. **Parallel dispatch** (if no circular dependencies):
   ...
   - Each subagent independently executes the full 12-step workflow and writes its outputs to `PROJECT_SCAFFOLD/docs/architecture/modules/{id}/`
...
5. **Conflict resolution**:
   ...
   - If conflicts found: present conflict list to user and enter **coordination mode**:
     1. **Conflict presentation**: Show a conflict matrix:
        ```
     ...
     3. **Fix sub-mode**:
        ...
        - Write fix diff to `PROJECT_SCAFFOLD/docs/architecture/validation/DESIGN_REVIEW_FIX_{conflict_id}.md`
...
- `PROJECT_SCAFFOLD/docs/architecture/modules/{module_id}/module-architecture.xml` (strict schema)
- `PROJECT_SCAFFOLD/docs/architecture/modules/{module_id}/module-interface-contract.md`
- `PROJECT_SCAFFOLD/docs/architecture/modules/{module_id}/components/{component_id}/component-spec.xml` (template for each component)
