# DevForge v1.4 Data Pipeline Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Promote data pipeline from isolated extension to core skill with full generation capabilities (dataflow.xml, ETL DAG, OLAP schema, data quality rules).

**Architecture:** The existing `extensions/data-pipeline-design/` becomes a reference-only extension providing evaluation dimensions. A new `devforge-data-pipeline` core skill handles all generation logic and integrates into the main chain.

**Tech Stack:** Markdown, YAML frontmatter, Python, SQL
...
|------|--------|---------------|
| `extensions/data-pipeline-design/SKILL.md` | Modify | Downgrade to reference-only extension |
| `devforge-data-pipeline/SKILL.md` | Create | New core skill for data pipeline generation |

---

### Task 1: Downgrade `extensions/data-pipeline-design` to Reference-Only

**Files:**
- Modify: `extensions/data-pipeline-design/SKILL.md`

**Required modifications:**

1. Update the description in YAML frontmatter from:
   ```yaml
...
2. Add a prominent note at the top of the markdown body (after `# Data Pipeline Design Extension`):
   ```markdown
...
3. Remove the "Interface Additions" section (lines 37-43) since interface definitions now belong to the core skill.

---

### Task 2: Create `devforge-data-pipeline` Core Skill

**Files:**
- Create: `devforge-data-pipeline/SKILL.md`
...
Expected: All skills validate successfully, including the new `devforge-data-pipeline` skill.
...
**Spec coverage check:**
- Downgrade existing extension → Task 1 ✓
- Create core skill with generation logic → Task 2 ✓
